...
public class ReservationsHandlerTests extends TestBase {
    ...
    void ifRequestHasInvalidTimestamp_failFastWithRequestCommitted_andDoNotAcquireHandlerLock() {
        ...
        var request = new ReservationRequest(UUID.randomUUID().toString(), reservation.getProductId(), reservation.getUserId(), 1, Instant.now().minusSeconds(1000));
    ...
    void ifFailedToAcquireHandlerLock_failFastWithRequestCommitted() {
        ...
        var request = new ReservationRequest(UUID.randomUUID().toString(), firstReservation.getProductId(), UUID.randomUUID().toString(), 1, Instant.now().plusSeconds(10));

        var acquireHandlerLock = locksService
            .tryLock("order_system:reservation_request_handlers", List.of(request.getRequestId()), "lockValue", Duration.ofSeconds(10))
        ;
        acquireHandlerLock
    ...
    void ifReservationAlreadyExisted_putReservation() {
        var hooks = new ConcurrentHashMap<String, List<String>>();
        var reservation = setupReservation(4, 1, 1, ReservationStatus.OK);
        var request = new ReservationRequest(UUID.randomUUID().toString(), reservation.getProductId(), reservation.getUserId(), 1, Instant.now().plusSeconds(10));
    ...
    void ifInSufficientStockForReservation_putReservationWithCorrectStatus() {
        var hooks = new ConcurrentHashMap<String, List<String>>();
        var firstReservation = setupReservation(4, 1, 1, ReservationStatus.OK);
        var request = new ReservationRequest(UUID.randomUUID().toString(), firstReservation.getProductId(), UUID.randomUUID().toString(), 5, Instant.now().plusSeconds(10));
    ...
    void ifStockSufficientForReservation_putReservationWithCorrectStatus() {
        var hooks = new ConcurrentHashMap<String, List<String>>();
        var firstReservation = setupReservation(4, 1, 1, ReservationStatus.OK);
        var request = new ReservationRequest(UUID.randomUUID().toString(), firstReservation.getProductId(), UUID.randomUUID().toString(), 2, Instant.now().plusSeconds(10));
    ...
    void ifNewReservation_andStockSufficient_updateProductAvailabilityAddByQuantity() {
        var reservation = setupReservation(4, 1, 1, ReservationStatus.OK);
        var request = new ReservationRequest(UUID.randomUUID().toString(), reservation.getProductId(), UUID.randomUUID().toString(), 1, Instant.now().plusSeconds(10));
    ...
    void ifExistingReservation_andStockSufficient_updateProductAvailabilityReplaceByQuantity() {
        var reservation = setupReservation(4, 1, 1, ReservationStatus.OK);
        var request = new ReservationRequest(UUID.randomUUID().toString(), reservation.getProductId(), reservation.getUserId(), 2, Instant.now().plusSeconds(10));
        var availability = handler
            .handle(request, null)
            .then(availabilitiesRepo.findByProductId(reservation.getProductId()))
            .block()
    ...
    void ifExistingReservation_andInSufficientStock_updateProductAvailabilityAddMaxAvailableQuantity() {
        var reservation = setupReservation(4, 1, 1, ReservationStatus.OK);
        var request = new ReservationRequest(UUID.randomUUID().toString(), reservation.getProductId(), reservation.getUserId(), 7, Instant.now().plusSeconds(10));
        var availability = handler
            .handle(request, null)
